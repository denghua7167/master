...
```{r maximum average daily activity, echo=TRUE}
Max_index<- which(Average_steps == max(Average_steps))
Max_interval<- data$interval_revised[Max_index]
```
The `r Max_interval`th 5-minute interval contains the maximum number of steps.
...
```
The total number of missing values in the dataset is `r Missing_value`.
...
```{r imputting missing value, echo=TRUE}
for(i in 0:60)
  {
   for(j in 1:288)
     {
...
```{r histogram revised data, echo=TRUE}
...
```{r Mean Median Revised, echo=TRUE}
Mean_steps_revised<- mean(T_steps_revised)
Median_steps_revised<- median(T_steps_revised)
```
For the revised data, the mean of the total number of steps taken per day is `r Mean_steps_revised`; and the median of the total number of steps taken per day is `r Median_steps_revised`. The mean and the median for the revised data are very close to the mean and the median for the original data. Therefore, imputing missing data has no big impact on the estimates of the total daily number of steps.

# Are there differences in activity patters between weekdays and weekends?

1. Create a new factor variable in the dataset with two levels - "weekday" and "weekend" indicating whether a given date is a weekday or weekend day.
```{r day factor, echo=TRUE}
